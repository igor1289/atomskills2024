...
async function getLessonTaskList(req, res) {
  if (req.params.topic_code && req.params.lesson_code)
    res.json(
      await taskService.getLessonTaskList(
        req.params.topic_code,
        req.params.lesson_code
      )
    );
}
...
async function getTaskPK(req, res) {
  console.log(req.params);
  res.json(await taskService.getTaskPK(req.params.code));
}
